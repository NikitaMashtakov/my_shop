...
export const CartState = ({children}: {children: React.ReactNode}) => {
    ...
    const plusProduct = (product: ICartProduct) => {
        const prodIndex =  cartProducts.findIndex(value => value.product.id === product.product.id)
   ...
   // const remove = () => setCartProducts()
    ...
    return (
        <CartContext.Provider value={{cartProducts, addProductToCart, plusProduct, removeProduct, minusProduct}}>
            {children}
